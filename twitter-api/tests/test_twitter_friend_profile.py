...
def search(query, limit):
    obj = ProfileClass()
    return obj.friendsprofile(query, limit)


@pytest.mark.parametrize("user_id, limit", [('realdonaldtrump', 30), ('katyperry', 40)])
def test_twitter_friends_profile(user_id, limit):
    results = search(user_id, limit)
