...
class CapsClient:
    ...
    def __init__(self, base_url=None):
        if base_url:
            ...
            self.base_url = os.environ.get('CAPS_URL', 'http://192.168.20.59:5000')
    ...
    """
    def get_proxy_random(self, country_code=None, type=ProxyType.SOCKS5, auth=ProxyAuth.NO):
        filters = dict()
        params = ''
        if country_code:
    ...
    """
    def mark_proxy_not_active(self,proxy):
        req = urllib.request.Request(url=self.ns_proxy+proxy.id,
                    headers={
        ...
        try:
            ...
            raise ProxyNotFoundException('Proxy '+proxy.id+' doesn\'t exist.')
        except URLError as e:
            raise ServiceUnreachableException('Service is down or you are not connected to the network.')
        pass
    ...
    """
    def mark_credential_not_active(self,credential):
        req = urllib.request.Request(url=self.ns_credential+credential.id,
                    headers={
                        'Accept':'application/json',
                        'Content-Type':'application/json'
                    },
        ...
        try:
            ...
            raise ProxyNotFoundException('Credential '+credential.id+' doesn\'t exist.')
        except URLError as e:
            raise ServiceUnreachableException('Service is down or you are not connected to the network.')
        pass
