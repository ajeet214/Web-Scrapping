...
class FollowersProfile:
    ...
    def redis_channel(self, selenium_session):
        ...
        client_id = 'twitter_followers_profile_' + selenium_session
    ...
    def test(self, next_cursor, username, client_id, redis_obj, channel_obj):
        ...
        url = f'https://api.twitter.com/1.1/followers/list.json?cursor={next_cursor}&screen_name={username}&skip_status=true&include_user_entities=false&count=199'
        try:
    ...
    def dataprocessing(self, followers_object, username, client_id, redis_obj, channel_obj):
        sleep(1)
        next_page, next_cursor = self.parser(followers_object, client_id, redis_obj)
    ...
    def followersprofile(self, username):

        url = f'https://api.twitter.com/1.1/followers/list.json?cursor=-1&screen_name={username}&skip_status=true&include_user_entities=false&count=199'
...
if __name__ == '__main__':
    tmp = FollowersProfile()
    # print((tmp.profilefetcher("realdonaldtrump")))
    print(tmp.followersprofile("sarapova"))
