...
class UserInterests:
    ...
    def user_interests(self, username, category):

        if category == 'influencers':
            url = f'https://www.linkedin.com/in/{username}/detail/interests/influencers'
        else:
            url = f'https://www.linkedin.com/in/{username}/detail/interests/{category}'
        print(url)

        self.driver.get(url)
        scroll_element = self.driver.execute_script('return document.querySelector(".entity-all").id')

        self.driver.execute_script(f"document.getElementById('{scroll_element}').scrollBy(0, document.getElementById('{scroll_element}').scrollHeight)")
        self.driver.execute_script(f"return document.getElementById('{scroll_element}').scrollHeight")

        self.driver.execute_script('''
            function myfunction(){
	            document.getElementById('ember122').scrollBy(0, document.getElementById('ember122').scrollHeight);
	            return document.getElementById('ember122').scrollHeight;
